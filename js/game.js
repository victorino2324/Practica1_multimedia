...
var game = {
	...
	handlePanning: function () {
		...
		if (game.mode == "firing") {
			if (mouse.down) {
			// Verificar que el héroe todavía existe antes de moverlo
			if (game.currentHero) {
				game.panTo(game.slingshotX);
			...
			// Verificar que el héroe todavía existe antes de lanzarlo
			if (game.currentHero) {
				game.mode = "fired";
		...
		for (var body = box2d.world.GetBodyList(); body; body = body.GetNext()) {
			...
			if (entity) {
				...
				if (outOfBounds || (entity.health && entity.health < 0)) {
					// DEBUGGING: Ver qué se está eliminando y por qué
					console.log("🔴 Eliminando:", entity.type, entity.name, 
						"| Pos:", Math.round(entityX), Math.round(entityY),
						"| OutOfBounds:", outOfBounds,
						"| Health:", entity.health,
						"| currentHero:", (game.currentHero === body),
						"| Modo:", game.mode);
	...
	drawSlingshotBand: function () {
		// Verificar que el héroe actual existe antes de dibujar la banda
		if (!game.currentHero) {
			return;
		}
...
var entities = {
	...
	create: function (entity) {
		...
		// Crear una COPIA LIMPIA de la entidad para no modificar el original
		var cleanEntity = {
			type: entity.type,
			name: entity.name,
			x: entity.x,
		...
		switch (entity.type) {
			...
			case "villain": // Pueden ser círculos o rectángulos
				...
				if (definition.shape == "circle") {
					cleanEntity.radius = definition.radius;
					box2d.createCircle(cleanEntity, definition);
				} else if (definition.shape == "rectangle") {
					cleanEntity.width = definition.width;
					cleanEntity.height = definition.height;
					box2d.createRectangle(cleanEntity, definition);
				}
				break;
			default:
...
var box2d = {
	scale: 30,
	init: function () {
		// Limpiar el mundo anterior si existe
		if (box2d.world) {
			// Destruir todos los cuerpos del mundo anterior
			for (var body = box2d.world.GetBodyList(); body; body = body.GetNext()) {
				var entity = body.GetUserData();
				if (entity && entity.groundTimer) {
					clearTimeout(entity.groundTimer);
					entity.groundTimer = undefined;
				}
			}
		...
		listener.PostSolve = function (contact, impulse) {
			...
			if (impulseAlongNormal > 5) {
				...
				// PERO NO reducir salud a héroes que aún NO han sido lanzados
				if (entity1.health) {
					// Solo dañar héroes que ya fueron lanzados o que no son héroes
					if (entity1.type !== "hero" || entity1.launched) {
				...
				if (entity2.health) {
					// Solo dañar héroes que ya fueron lanzados o que no son héroes
					if (entity2.type !== "hero" || entity2.launched) {
